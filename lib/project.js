import { createTreeStore, initModelView, isContextSwitchedCmd, isModelChangedCmd, isDiffableCmd, isEntryChangedCmd } from "./main.js"
...
const createProjectStore = () => createTreeStore({
    ...
    if (fmodel._diff[Diff.MOVED]) {
      ...
      switch (projectStore.taggedLevel[oldParent.level]) {
        ...
        case FILE_TAG:
          ...
          var { fields, models, allowedSchs, schMetas, ...file_ } = (file)
          changed.files[file._meta.path] = file_
        ...
        case T.FMODEL_TAG:
          ...
          changed.fmodels[foundFile.fmodelDst._meta.path] = { ...(foundFile.fmodelDst), parentAnchor: file_.$anchor }
          break
        default:
          ...
          var file_ = (foundFile.file)
          changed.fmodels[fmodelDst._meta.path] = { ...(fmodelDst), parentAnchor: file_.$anchor }
      }
    ...
    if (subfmodel._diff[Diff.MOVED]) {
      ...
      switch (projectStore.taggedLevel[newParent.level]) {
        ...
        case FILE_TAG:
          ...
          added.fmodels[subfmodel._meta.path] = { ...(subfmodel), parentAnchor: file_.$anchor }
          break
        case T.FMODEL_TAG:
        default:
          var fmodelDst = findFmodel(subfmodel._meta.path)
...
const handleProjectContext = (projectStore, target, command) => {
    ...
    switch (true) {
      case isEntryChangedCmd(command.name):
        ...
        walkFmodel(projectStore, (fmodel, m) => {
          if (m.path == `[${fileStore.key}]${target.id}`) {
        ...
        fmodelTree = document.querySelector("[id='fmodel'] [role='tree']")
        fmodelTree._render(fileStore)
        break
      case isModelChangedCmd(command.name):
        fileStore = getFileStore(projectStore, filename)
...
const fileToStore = (file, store, projectStore) => {
  for (let fmodel of file.fmodels) {
    ...
    // optional
    if (fmodel.isEntry) {
      fmodel.sch.isEntry = fmodel.isEntry
...
const walkFmodel = (projectStore, fn) => {
  for (let f of Object.keys(projectStore.fields)) {
    ...
    for (let m of Object.keys(file.fields)) {
      ...
      fn(fmodel, { path: `[${file.key}][${fmodel.key}]` })
    }
  }
}
