...
const handleProjectRemote = (projectStore, command, f) => {
  ...
  const simpleFmodel = ({ _diff, allowedSchs, ...fmodel }) => fmodel

  let diffs = projectStore._diff
...
const handleProjectContext = (projectStore, target, command, runDiff) => {
  ...
  if (target.closest("[id='project']"))
    switch (true) {
      case filename && isContextSwitchedCmd(command.name):
        projectStore._diff = runDiff()
        changeFile(projectStore, filename)
        document
          .querySelector("[id='project'] [role='tree']")
          ._render(projectStore)
        break
