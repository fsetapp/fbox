...
export const getFileStore = (projectStore, filename) => {
  let fileStore = Sch.get(projectStore, `[${filename}]`) || projectStore._currentFileStore
  return fileStore
}
...
export const anchorsModels = (projectStore) => {
  ...
  walkFmodel(projectStore, (fmodel, m) => {
    modelsAcc[fmodel.$a] = { display: [m.file.key, fmodel.key].join(" :: "), file: m.file.key, fmodel: fmodel.key, type: fmodel.t }
  })
  return modelsAcc
}
...
export const allSchs = [
  ...
  T.taggedUnion, T.dict, T.erecord
]
...
export const rulesTable = [dictRule, tagUnionRule, erecordRule]
