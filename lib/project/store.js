...
const createTreeStore = (opts = {}) => {
  ...
  readable(s, "allowedSchs", opts.allowedSchs || allSchs.map(sch => sch()))

  if (opts.taggedLevel)
    readable(s, "taggedLevel", opts.taggedLevel)

  if (opts.put)
    readable(s, "put", opts.put)

  if (opts.entryable)
    readable(s, "entryable", opts.entryable)

  if (opts.addDefault)
    readable(s, "_addSchDefault", s.allowedSchs.find(sch_ => sch_.type == opts.addDefault().type) && opts.addDefault())
