...
import { readable, forEach, find, reduce } from "../utils.js"
...
export const taggedDiff = (projectStore, f) => {
  ...
  const simpleFile = ({ $a, key, tag, index, t, _diff }) => {
    let lapth = _diff.meta.lpath.map(({ key, t, $a }) => ({ key, t, $a }))
    return ({ $a, key, tag, index, t, lapth })
  }
  ...
  for (let sch of diffs) {
    if (sch.t == T.PROJECT) toUpdateSchs.project = sch
    else if (sch.t == T.FILE) toUpdateSchs.files.push(sch)
    else if (sch.t == T.FOLDER) continue
    else if (sch._diff.meta.pt == T.FILE) toUpdateSchs.fmodels.push(sch)
  ...
  for (let i = 0; i < toUpdateSchs.fmodels.length; i++) {
    ...
    // Note: file order update is in `changed.files` scope, associated fact in diff scope is quite fragile
      ...
      let fileOf = fmodel._diff.meta.lpath.find(p => p.t == T.FILE)
      ...
      switch (oldParent.tag) {
        ...
        default:
          staleFile = oldParent.lpath.find(a => a.t == T.FILE)
          staleFmodel = staleFmodel || oldParent.lpath.find(a => a.tag == T.FMODEL_TAG)
  ...
  for (let i = 0; i < toUpdateSchs.subfmodels.length; i++) {
    ...
    if (subfmodel._diff[Diff.NEW] ||
      ...
      staleFile = subfmodel._diff.meta.lpath.find(a => a.t == T.FILE)
      staleFmodel = subfmodel._diff.meta.lpath.find(a => a.tag == T.FMODEL_TAG)
      ...
      switch (newParent.tag) {
        ...
        case T.FMODEL_TAG:
          fmodelOf = newParent
        default:
          fileOf = newParent.lpath.find(a => a.t == T.FILE)
          fmodelOf = fmodelOf || newParent.lpath.find(a => a.tag == T.FMODEL_TAG)
