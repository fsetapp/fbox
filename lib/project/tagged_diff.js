...
import { PROJECT, FILE, FOLDER, FILE_TAG, PROJECT_TAG } from "../pkgs/proj.js"
...
export const taggedDiff = (projectStore, f) => {
  ...
  for (let sch of diffs) {
    ...
    if (sch.t == PROJECT) toUpdateSchs.project = sch
    else if (sch.t == FILE) toUpdateSchs.files.push(sch)
    else if (sch.t == FOLDER) continue
    else if (sch._diff.meta.pt == FILE) toUpdateSchs.fmodels.push(sch)
  ...
  for (let i = 0; i < toUpdateSchs.fmodels.length; i++) {
    ...
    // Note: file order update is in `changed.files` scope, associated fact in diff scope is quite fragile
      ...
      let fileOf = fmodel._diff.meta.lpath.find(p => p.t == FILE)
      ...
      switch (oldParent.tag) {
        ...
        default:
          staleFile = oldParent.lpath.find(a => a.t == FILE)
  ...
  for (let i = 0; i < toUpdateSchs.subfmodels.length; i++) {
    ...
    if (subfmodel._diff[Diff.NEW] ||
      ...
      staleFile = subfmodel._diff.meta.lpath.find(a => a.t == FILE)
      ...
      switch (newParent.tag) {
        ...
        case TOPLV_TAG:
          fmodelOf = newParent
        default:
          fileOf = newParent.lpath.find(a => a.t == FILE)
