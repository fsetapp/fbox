...
export const taggedDiff = (projectStore, f) => {
  ...
  for (let i = 0; i < toUpdateSchs.fmodels.length; i++) {
    ...
    // Note: file order update is in `changed.files` scope, associated fact in diff scope is quite fragile
      ...
      if (!reorder.fmodels[`[${foundFile.file.key}][${foundFile.fmodelDst.key}]`])
        forEach(foundFile.file.fields, (fmodel, i) => {
          reorder.fmodels[`[${foundFile.file.key}][${fmodel.key}]`] = {
            $a: fmodel.$a, t: fmodel.t, key: fmodel.key, index: i, pa: foundFile.file.$a
          }
        })
    }
  ...
  for (let i = 0; i < toUpdateSchs.subfmodels.length; i++) {
    ...
    if (subfmodel._diff[Diff.NEW] ||
      ...
      let fileKey = subfmodel._diff.meta.lpath[0]
      let fmodelKey = subfmodel._diff.meta.lpath[1]
      let fmodelPath = `[${fileKey}][${fmodelKey}]`

      if (!added.fmodels[fmodelPath] && !removed.fmodels[fmodelPath]) {
        let foundFile = findFile(fmodelKey)
        let fmodelDst = foundFile.fmodelDst

        changed.fmodels[fmodelPath] = assign(simpleFmodel(fmodelDst), { pa: foundFile.file.$a })
      }
    }
      ...
      switch (projectStore.taggedLevel[oldParent.level]) {
        ...
        case T.FMODEL_TAG:
        default:
          var foundFile = findFile(oldParent.lpath[1])
          var fmodelDst = foundFile.fmodelDst
          var fmodelPath = `[${foundFile.file.key}][${fmodelDst.key}]`

          var file_ = simpleFile(foundFile.file)
          changed.fmodels[fmodelPath] = assign(simpleFmodel(fmodelDst), { pa: file_.$a })
      }
      ...
      switch (projectStore.taggedLevel[newParent.level]) {
        ...
        case T.FMODEL_TAG:
        default:
          var foundFile = findFile(newParent.lpath[1])
          var fmodelDst = foundFile.fmodelDst
          var fmodelPath = `[${foundFile.file.key}][${fmodelDst.key}]`
