...
const fmodelController = (projectStore, filename, target, command, runDiff) => {
  ...
  switch (command) {
    case toggleInlineDocs:
      changeFile({ projectStore, filename, inlineDocs: fileStore.inlineDocs, focus: true })
      break
  }
...
export const changeFile = (conf) => {
  let { projectStore, filename, fmodelname = null, focus = false, inlineDocs } = conf
  let fileStore = getFileStore(projectStore, filename)

  if (fileStore) {
    readable(projectStore, "_currentFileStore", fileStore)
    fileStore._models = anchorsModels(projectStore)
    FmodelTree({ store: fileStore, target: "[id='fmodel']", select: fmodelname, focus, inlineDocs })
