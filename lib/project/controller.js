...
export const controller = (projectStore, target, command, runDiff) => {
  ...
  if (target.closest("[id='project']"))
    projectController(projectStore, filename, target, command, runDiff)
  else if (target.closest("[id='fmodel']"))
    fmodelController(projectStore, filename, target, command, runDiff)
}

const projectController = (projectStore, filename, target, command, runDiff) => {
  ...
  switch (command) {
    ...
    case selectDown:
    case selectLast:
      changeFile({ projectStore, filename, target })
      break
...
export const changeFile = (conf) => {
  let { projectStore, filename, target, fmodelname = null, focus = false } = conf

  let fileStore = getFileStore(projectStore, filename, target)

  if (target?.dataset?.tag == FILE_TAG && fileStore) {
