...
export const edit = ({ store, scope, e }) => {
  ...
  const sch = Sch.update(store, (a, m) => a.$a == header.dataset.anchor, (a, m) => readable(a, "uiMode", "edit"))
...
export const save = ({ store, scope, e }) => {
  ...
  const sch = Sch.update(store, (a, m) => a.$a == form.dataset.anchor, (a, m) => readable(a, "uiMode", "saved"))
  store.renderSchMeta(sch)
