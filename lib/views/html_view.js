...
const viewTreeitem = (assigns) => {
  const treeitem = ({ sch, ui, key, path }, itemView) =>
    html`<li id="${path}" class="${ui.nodeclass} ${tagText(sch, ui)}" .key="${key}" .index="${ui.index}" .sch="${sch}" data-diff="${diffdata(sch._diff)}" data-sig="${errordata(sch, ui)}" data-tag="${ifTagdata(sch.tag)}" role="treeitem" aria-level="${ui.level}" aria-posinset="${ui.index + 1}" aria-selected="false" tabindex="-1" aria-expanded="${ifDefined(ui.expanded)}">${itemView(assigns)}</li>`

  switch (true) {
    case assigns.ui.level == assigns.ui.rootLevel:
      Object.assign(assigns.ui, { nodeclass: "root-item", index: 0, expanded: true })
      return treeitem(assigns, viewRootItem)

    case Html.phrasingContent[assigns.sch.t]:
      Object.assign(assigns.ui, { nodeclass: "leaf", flow: "inline" })
      return treeitem(assigns, viewLeaf)

    case assigns.sch.hasOwnProperty("fields") || assigns.sch.hasOwnProperty("schs"):
      Object.assign(assigns.ui, { nodeclass: "node", expanded: true })
      return treeitem(assigns, viewNode)

    default:
      Object.assign(assigns.ui, { nodeclass: "leaf" })
      return treeitem(assigns, viewLeaf)
  }
}
...
const editableVal = (assigns) =>
  (assigns.sch.uiMode == "editVal") ?
    textInput("val-edit", assigns.ui.parentPath, assigns.key) :
    html`${assigns.sch.v}`

const editableText = (assigns) => {
  switch (true) {
    case assigns.sch.uiMode == "editVal":
      return html`<combo-box class="v" id="type_search" list="typesearch">
