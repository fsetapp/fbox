...
import { TFOOT } from "../pkgs/html.js"
...
export const renderRoot = (el, root, opts = {}) => {
  try {
    let ui = {
      ...
      structSheet: root.structSheet, rootHead: { t: root.t, ext: root.ext }
    }
...
const viewNode = (assigns) =>
  html`
  <li id="${assigns.path}" data-diff="${diffdata(assigns.sch._diff)}" data-tag="${ifTagdata(assigns.sch.tag)}" .key="${assigns.key}" .index="${assigns.ui.index}" .sch=${assigns.sch} aria-posinset="${assigns.ui.index + 1}" class="node" role="treeitem" data-level-kind="${assigns.ui.level % 2 == 0 ? 'even' : 'odd'}" aria-level="${assigns.ui.level}" aria-selected="false" aria-expanded="${assigns.ui.expanded}" tabindex="-1">
    <dfn class="h" data-group-size="${groupSize(assigns.sch)}">
      ${viewType(assigns)}
    </dfn>
...
const viewLeaf = (assigns) =>
  html`
  <li id="${assigns.path}" data-tag="${ifTagdata(assigns.sch.tag)}" data-diff="${diffdata(assigns.sch._diff)}" .key="${assigns.key}" .index="${assigns.ui.index}" .sch=${assigns.sch} aria-posinset="${assigns.ui.index + 1}" class="leaf" role="treeitem" data-level-kind="${assigns.ui.level % 2 == 0 ? 'even' : 'odd'}" aria-level="${assigns.ui.level}" aria-selected="false" tabindex="-1">
    ${viewType(assigns)}
  </li>`
...
const typeText = (sch, ui) => {
  const { toStr } = ui.structSheet[ui.rootHead.ext]
  return toStr(sch.t)
}
