...
export const renderMeta = (container, sch, selected, store) => {
  let referrers = sch.referrers || []
  container = document.querySelector(container)
  container.classList.remove("hidden")

  try {
    render(html`<div data-file=${store.key} .file="${store}">${docs(sch, selected, store)}</div>`, container)
  }
  catch (e) { console.log(e); }
}

const docs = (sch, active, store) =>
  html`
  <p class=""></p>
