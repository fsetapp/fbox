...
export const textInput = (sch, key, opts = {}) =>
  labelA(sch, key, html`
    <textarea name="${name(sch, key)}" maxlength="${zeroable(opts.maxlength)}" minlength="${zeroable(opts.minlength)}" ?readonly="${opts.readonly}" rows="1" spellcheck="false" data-newline="false" class="${ifDefined(errors(sch, key) && "invalid")}" @input="${autoResize}" .value=${opts.value || value(sch, key, "")}></textarea>
  `, opts)
