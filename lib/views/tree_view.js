...
const viewRootItem = (assigns) =>
  html.for(assigns.parent, assigns.sch.$a)`
...
const viewKeyNonTop = (assigns) => {
  switch (true) {
    ...
    case assigns.parent.t == T.RECORD:
      let metadata = assigns.ui.schMetas[assigns.sch.$a]
      return html`
...
const typeText = (sch, ui) => {
  let text
  let metadata = ui.schMetas[sch.$a]
  ...
  switch (true) {
    ...
    case sch.t == T.REF && !!ui.models[sch.$r]: text = html`<span class="ref">${sch._text = typeTextPopulated(sch, ui)}</span>`; break
