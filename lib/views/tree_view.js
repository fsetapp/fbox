...
const indent = (assigns, extra = 1) => `padding-left: ${((assigns.ui.level - 1) * 2) + assigns.ui.tab - 1 + extra}ch`
...
const viewKeyNonTop = (assigns) => {
  switch (true) {
    case assigns.parent.type == T.UNION:
      return html`
      <span class="s" style="${indent(assigns, 2)}">|</span>
      `
    ...
    case assigns.parent.type == T.RECORD:
      ...
      return html`
      <span class="k" style="${indent(assigns, 2)}" .dataset="${{ required: metadata?.required }}">
        ${editableKey(assigns)}
      </span>
      <span class="s">:</span>
    `
    ...
    default:
      return html`
      <span class="k" style="${indent(assigns, 2)}">
        ${editableKey(assigns)}
      </span >
      <span class="s">:</span>
    `
