...
const viewMain = (el, assigns) =>
  ...
  <ul role="tree" aria-multiselectable="true" class="${`${assigns.ui.lineClass || ''} text-sm`}" aria-labelledby="${assigns.ui.taggedLevel[assigns.ui.level] + "_tree"}">
    ${assigns.ui.nothing ? nothing : viewModel(assigns)}
  </ul>
...
const viewRootItem = (assigns) =>
  html`
  <li id="${assigns.path}" data-tag="${assigns.ui.taggedLevel[assigns.ui.level]}" .key="${assigns.key}" .index="${0}" aria-posinset="${1}" class="root-item" role="treeitem" aria-level="${assigns.ui.level}" aria-selected="false" aria-expanded="${assigns.ui.expanded}" tabindex="-1">
...
const viewFolder = (assigns) =>
  html`
  <li id="${assigns.path}" data-tag="${assigns.ui.taggedLevel[assigns.ui.level]}" .key="${assigns.key}" .index="${assigns.ui.index}" aria-posinset="${assigns.ui.index + 1}" class="folder" role="treeitem" aria-level="${assigns.ui.level}" aria-selected="false" aria-expanded="${assigns.ui.expanded}" tabindex="-1">
...
const viewFile = (assigns) =>
  html`
  <li id="${assigns.path}" data-tag="${assigns.ui.taggedLevel[assigns.ui.level]}" data-diff="${diffdata(assigns.sch._diff)}" .key="${assigns.key}" .index="${assigns.ui.index}" aria-posinset="${assigns.ui.index + 1}" class="file" role="treeitem" aria-level="${assigns.ui.level}" aria-selected="false" tabindex="-1">
    ${viewKey(Object.assign(assigns, { key: wordBreakHtml(assigns.key) }))}
    ${viewType(assigns)}
  </li>`
...
const viewKeyNonTop = (assigns) => {
  switch (true) {
    ...
    case assigns.parent.t == T.RECORD:
      ...
      return html`
      <span class="k" style="${indent(assigns, 1)}" data-required="${!!assigns.sch.metadata?.required}">
        ${editableKey(assigns)}
      </span>
      <span class="s">:</span>
    `
