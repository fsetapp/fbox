  ...
  try {
    if (!root._models) {
      ...
      for (let i = 0; i < root.fields.length; i++)
        root._models[root.fields[i].$anchor] = root.fields[i].key
    }

    let ui = { level: 1, tab: opts?.tab || 0, models: root._models, schMetas: root.schMetas, rootKey: root.key, taggedLevel: root.taggedLevel }
...
export const viewMain = (el, assigns) =>
  ...
  <ul role="tree" aria-multiselectable="true" class="text-sm" aria-labelledby="${assigns.ui.taggedLevel[assigns.ui.level] + "_tree"}">
    ${viewModel(assigns)}
  </ul>
  ${null && debug(assigns)}
`)
...
const editableType = (assigns) =>
  (assigns.sch.uiMode == "editType") ?
    html`<combo-box .models="${{ list: assigns.ui.models }}">
...
const typeText = (sch, ui) => {
  ...
  switch (true) {
    ...
    case sch.type == T.REF && !!ui.models[sch.$ref]: text = html`<span class="ref">${sch._text = ui.models[sch.$ref].split(".").join(" :: ")}</span>`; break
