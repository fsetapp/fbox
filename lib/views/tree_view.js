...
const viewFile = (assigns) =>
  ...
  ${inlineDocs(assigns)}
  `
const inlineDocs = (assigns) => {
  if (assigns.ui.inlineDocs)
    return assigns.sch.metadata?.description &&
      html`<pre style="${tunedIndent(assigns)}" class="inline-docs">${assigns.sch.metadata.description}</pre>`
  else
    return nothing
}
...
const tunedIndent = (assigns, extra = 0) => {
  switch (true) {
    case assigns.parent.t == T.UNION: extra = 3.5; break
    case assigns.parent.t == T.RECORD: extra = 3.5; break
    case assigns.parent.t == T.TAGGED_UNION: extra = 0.5; break
    default: extra = 1
  }
  return `padding-left: ${((assigns.ui.level - 1) * 2) + assigns.ui.tab - 1 + extra}ch`
}
...
const typeText = (sch, ui) => {
  ...
  switch (true) {
    ...
    case sch.t == T.STRING:
      text = "string"
      if (metadata?.pattern) text = html`${text} <span class="m pattern">/${metadata.pattern}/<span>`
      break
