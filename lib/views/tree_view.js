...
const diffdata = (_diff) => {
  ...
  if (!_diff) return ifDefined(diffchar)
...
const viewFolder = (assigns) =>
  html`
  <li id="${assigns.path}" data-tag="${ifTagdata(assigns.ui)}" .key="${assigns.key}" .index="${assigns.ui.index}" aria-posinset="${assigns.ui.index + 1}" class="folder" role="treeitem" aria-level="${assigns.ui.level}" aria-selected="false" aria-expanded="${assigns.ui.expanded}" tabindex="-1">
...
const viewFile = (assigns) =>
  html`
  <li id="${assigns.path}" data-tag="${ifTagdata(assigns.ui)}" data-diff="${diffdata(assigns.sch._diff)}" .key="${assigns.key}" .index="${assigns.ui.index}" aria-posinset="${assigns.ui.index + 1}" class="file" role="treeitem" aria-level="${assigns.ui.level}" aria-selected="false" tabindex="-1">
    ${viewKey(Object.assign(assigns, { key: wordBreakHtml(assigns.key) }))}
    ${viewType(assigns)}
  </li>`
