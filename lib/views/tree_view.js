...
const viewItself = (assigns) => {
  ...
  switch (true) {
    ...
    case [T.TUPLE, T.DICT].includes(assigns.sch.type):
...
const viewKeyNonTop = (assigns) => {
  switch (true) {
    ...
    case assigns.parent.type == T.RECORD:
      ...
      return html`
      <span class="k" style="${indent(assigns)}" .dataset="${{ required: metadata?.required }}">
        ${editableKey(assigns)}
      </span>
      <span class="s">:</span>
    `
    case assigns.parent.type == T.TAGGED_UNION:
      return html`
      <span class="s" style="${indent(assigns)}">| t </span>
      <span class="k tag">
        ${editableKey(assigns)}
      </span>
      <span class="s"></span>
