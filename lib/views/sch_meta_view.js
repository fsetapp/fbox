...
export const renderMeta = (container, sch, root) => {
  try {
    ...
    <form data-path="${sch.path}" data-file="${sch.rootKey}" class="fbox">
      ${textInput(sch, "title", { keyDisplay: "Title" })}
      ${textInput(sch, "description", { keyDisplay: "Description" })}
      ${renderTypeMeta(sch)}
      ${enumInput(sch, "rw", { collection: [["r", "Readonly"], ["w", "Writeonly"], ["rw", "Read and Write"]], selected: value(sch, "rw"), keyDisplay: "Read / Write" })}
...
const renderTypeMeta = (sch) => {
  ...
  let hasParentRecord = sch._meta.parent.type == T.RECORD
