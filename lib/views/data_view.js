...
const valInput = assigns => {
  ...
  const value = o("default") || val(sch)
  switch (true) {
    case sch.t == M.STRING:
      return html`<input type="text" minLength="${o("minLength")}" maxLength="${o("maxLength")}" pattern="${o("pattern")}" .value="${value}" @input="${autoWidth(2)}"></input>`
