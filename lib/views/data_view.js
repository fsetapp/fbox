...
import { autoResize, autoWidth, cursorEnd, writable } from "../utils.js"
  ...
  switch (true) {
    ...
    case is(parent, S.data()):
      ...
      if (parent.schs[1]?.$a == sch.$a) {
        if (ui._columns[sch.$a] && ui.columnIndex != ui._columns[sch.$a].ui.columnIndex) {
          return nothing
        }

        assigns.item = { nodeclass: `data ctor-body ${typestr}`, expanded: true }
    ...
    case isContainer(sch):
      assigns.item = { nodeclass: `node data ${typestr}`, expanded: false }
...
const viewNode = assigns =>
  html`
  <div class="h" style="${indent(assigns)}" data-diff="${diffdata(assigns.sch._diff)}" data-group-size="${groupSize(assigns.sch)}">
    ${viewKST(assigns)}
  </div>
...
const newItemBtn = assigns => {
  ...
  switch (true) {
    case is(sch, M.list()):
      return html`<button data-cmd="new-item" style="${indentm(assigns)}">+ New ${sch.sch.key}</button>`
    case is(sch, M.dict()):
      return html`<button data-cmd="new-item" style="${indentm(assigns)}">+ New ${sch.key}</button>`
    default:
      return nothing
  }
}
...
export const viewKST = assigns => {
  ...
  switch (true) {
    ...
    default:
      // leaf
      return html`<span class="k" style="${indent(assigns)}">${assigns.key}</span>${valInput(assigns)}`
  }
}
...
const valInput = assigns => {
  ...
  switch (true) {
    case is(sch, M.string()):
      return html`<input type="text" .value="${val(sch)}" data-cmd="validate" @input="${autoWidth(2)}"></input>`
    case is(sch, M.bool()):
    ...
    case is(sch, M.uint32()):
      ...
      return html`<input type="number" @input="${autoWidth(5)}" min=${range.min} max=${range.max} .value="${val(sch)}" data-cmd="validate"></input>`
    case is(sch, M.integer()):
      return html`<input type="number" @input="${autoWidth(5)}" min=${ifDefined(sch?.metadata?.min)} max=${ifDefined(sch?.metadata?.max)} .value="${val(sch)}" data-cmd="validate"></input>`
