...
import { K as M } from "../pkgs/model.js"
  ...
  switch (true) {
    ...
    case [M.LIST].includes(assigns.sch.t):
      if (!!sch.sch.fields || !!sch.sch.schs)
        ...
        assigns.item = { nodeclass: "node", expanded: b(sch.expanded, false) }
...
const editableKey = (assigns) =>
  (assigns.sch.uiMode == "editKey") ?
    html`<span>${textInput("key-edit", assigns.ui.parentId, assigns.key)}</span>` :
    html`${assigns.key}`
...
const editableType = (assigns) =>
  (assigns.sch.uiMode == "editType") ?
    html`<combo-box id="type_search" list="typesearch" class="t">
      ${textInput("type-edit", assigns.ui.parentId, typeTextPopulated(assigns.sch, assigns.ui))}
    </combo-box>` :
