...
const iterate = (collection, keyFn, pathFn, assigns) =>
  repeat(assigns.ui.itemsFn(collection), (sch) => sch.$a, (sch, i) =>
    assigns.ui.view({
      ...
      ui: {
        ...assigns.ui,
        level: assigns.ui.level + 1,
        index: i,
        parentPath: assigns.path,
        parentId: assigns.sch.$a,
        lPath: lPath(assigns.ui.lPath || [], readable(sch, "i", i) || sch)
      },
...
export const treeItem = (assigns, itemView) => {
  ...
  return html`<li id="${sch.$a}" class="${item?.nodeclass}" .key="${key}" .index="${ui.index || 0}" .sch="${sch}" data-diff="${diffdata(sch._diff)}" data-tag="${ifTagdata(sch.tag)}" role="treeitem" aria-level="${ui.level}" aria-posinset="${(ui.index || 0) + 1}" aria-selected="${sch.selected || false}" tabindex="-1" aria-expanded="${ifDefined(item?.expanded)}">${itemView(assigns)}</li>`
}
