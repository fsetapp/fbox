...
export const roleTree = (el, root, view, opts = {}) => {
  try {
    ...
    ui.columns = Object.values(root._columns)
    ...
    let tree = html`<ol role="tree" aria-multiselectable="true" class="${`${ui.lineClass || ''} text-sm`}" aria-label="${root.key + " body"}">
      ${repeat(ui.columns, assigns => assigns.sch.$a, (assigns, i) => view(assigns))}
    </ol>`
...
export const treeItem = (assigns, itemView) => {
  ...
  return html`<li id="${path}" class="${item?.nodeclass}" .key="${key}" .index="${ui.index || 0}" .sch="${sch}" data-diff="${diffdata(sch._diff)}" data-sig="${errordata(sch, ui)}" data-tag="${ifTagdata(sch.tag)}" role="treeitem" aria-level="${ui.level}" aria-posinset="${(ui.index || 0) + 1}" aria-selected="false" tabindex="-1" aria-expanded="${ifDefined(item?.expanded)}">${itemView(assigns)}</li>`
}
