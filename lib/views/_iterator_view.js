...
export const treeItem = (assigns, itemView) => {
  ...
  return html`<li id="${path}" class="${item?.nodeclass}" .key="${key}" .index="${ui.index || 0}" .sch="${sch}" data-diff="${diffdata(sch._diff)}" data-tag="${ifTagdata(sch.tag)}" role="treeitem" aria-level="${ui.level}" aria-posinset="${(ui.index || 0) + 1}" aria-selected="${sch.selected || false}" tabindex="-1" aria-expanded="${ifDefined(item?.expanded)}">${itemView(assigns)}</li>`
}
