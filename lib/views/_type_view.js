...
const isCoreRef = t => t[0] == CorePkg && t[1] == Core.REF
...
export const typeText = (sch, ui) => {
  ...
  switch (true) {
    ...
    case isCoreRef(sch.t) && !!ui.models[sch.$r]:
      writable(sch, "_text", typeTextPopulated(sch, ui))
      text = html`<span class="ref" ._a="${sch.$r}">${sch._text}</span >`
      break
    case isCoreRef(sch.t) && sch.$r == null: text = html`<span class="ref" title="Ref type">{ }</span>`; break
    case isCoreRef(sch.t): text = html`<span class="ref notfound" title="Ref type">${`${sch._text || sch.$r} (#404)`}</span>`; break
