...
export const ProjectTree = ({ store, target, select, focus = true, inlineDocs = false }) => {
  TreeComponent({
    store,
    target,
    treeKeyDownCmds: [
...
export const FmodelTree = (conf = {}) => {
  ...
  TreeComponent({
    ...
    view: (s) => renderRoot(target, s || store, {
      ui: {
        lineClass: conf.lineNumber || "setnu",
        inlineDocs: conf.inlineDocs,
        tab: conf.lineNumber ? 1 : 3,
