...
export const FmodelTree = ({ store, target, select, focus }) => {
  // Rendering nothing wipes out all html, so DOM diffs with nothing
  // which is about 4x faster (1600ms vs 400ms)
  renderRoot(target, store, { ui: { nothing: true } })
  TreeComponent({
    store,
    target,
    treeKeyDownCmds: [
...
export const SchMetaForm = ({ store, target, treeTarget }) => {
  ...
  readable(store, "renderSchMeta", function (s) {
    renderSchMeta({ tree, store: s || this, metaForm })
  })
  store.renderSchMeta()
}
