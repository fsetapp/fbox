 ...
 */
const currentMeta = ({ m, t, $a, key, v, tag }, { path, lpath, level }) =>
  ...
  switch (true) {
    case sch.hasOwnProperty("fields"):
      forEach(sch.fields, (sch_, i) => {
        let k = sch_.key
        let nextMeta = { path: `${meta.path}[${k}]`, level: meta.level + 1, index: i, parent: currentMeta(sch, meta), lpath: lPath(meta.lpath, sch_, meta) }
    ...
    case sch.hasOwnProperty("schs"):
      forEach(sch.schs, (sch_, i) => {
        let nextMeta = { path: `${meta.path}[][${i}]`, level: meta.level + 1, index: i, parent: currentMeta(sch, meta), lpath: lPath(meta.lpath, sch_, meta) }
    ...
    case sch.hasOwnProperty("sch"):
      let sch_ = sch.sch
      let nextMeta = { path: `${meta.path}[][0]`, level: meta.level + 1, index: 0, parent: currentMeta(sch, meta), lpath: lPath(meta.lpath, sch_, meta) }
