 ...
 */
const walk = (sch, f, meta = { path: "", lpath: [], level: 1, parent: {} }) => {
  delete sch._halt
  delete sch._pruned
  sch = f(sch, meta)
  ...
  switch (true) {
    case sch.hasOwnProperty("fields"):
      for (let i = 0; i < sch.fields.length; i++) {
        ...
        let k = sch_.key
        let nextMeta = { path: `${meta.path}[${k}]`, level: meta.level + 1, index: i, parent: currentMeta(sch, meta), lpath: meta.lpath.concat([k]) }
    ...
    case sch.hasOwnProperty("schs"):
      for (let i = 0; i < sch.schs.length; i++) {
        let sch_ = sch.schs[i]
        let nextMeta = { path: `${meta.path}[][${i}]`, level: meta.level + 1, index: i, parent: currentMeta(sch, meta), lpath: meta.lpath.concat([i]) }
    ...
    case sch.hasOwnProperty("sch"):
      let nextMeta = { path: `${meta.path}[][0]`, level: meta.level + 1, index: 0, parent: currentMeta(sch, meta), lpath: meta.lpath.concat([0]) }
      let sch_ = sch.sch
