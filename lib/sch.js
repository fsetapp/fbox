...
const simplify = (sch) =>
  walk(JSON.parse(JSON.stringify(sch)), (a, m) => {
    let { type, $anchor, key, _tag, isEntry, parentAnchor } = a
    let common = { type, $anchor, key, _tag, parentAnchor }
...
const walk = (sch, f, meta = { path: "", level: 1, parent: {} }) => {
  ...
  const currentMeta = ({ _tag, type, $anchor, key }, { path, level, parent: { prune } }) =>
    ({ _tag, level, type, $anchor, key, path, prune })
