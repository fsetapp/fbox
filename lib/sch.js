 ...
 */
const currentMeta = ({ t, $a, key, tag }, { path, lpath, level }) =>
  ({ t, $a, key, tag, level, path, lpath })
    ...
    if (typeof path == "function" && !path.call(null, sch_, meta)) return sch_
      ...
      switch (true) {
        ...
        case sch_.hasOwnProperty("schs"):
          // TODO: if (sch_.schs.length - descIndices.length >= sch_.minItem)
          if (sch_.schs.length - descIndices.length > 0)
            for (let i of descIndices)
...
const put = (schema, path, rawSchs) => {
  ...
  const ChildrenTag = (sch) => {
    for (let row of schema.structSheet?.sheet || [])
      switch (true) {
        case match(sch, row.selector):
          return row.rules.children?.tag
      }
  }
  const selfTag = (sch) => {
    for (let row of schema.structSheet?.sheet || [])
      ...
      switch (true) {
        case sch_.hasOwnProperty("fields"):
          ...
          for (let { k, sch, index } of ascRawSchs) {
            ...
            let tag = selfTag(newSch) || childTag
