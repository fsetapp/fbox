...
export { get, update, put, putSelected, pop, popToRawSchs, copyToRawSchs, putSelectedRawSchs, putPoppedRawSchs, move, changeType, clone }
  ...
  switch (true) {
    case [T.RECORD].includes(sch.type):
      ...
      for (let i = keys.length - 1; i >= 0; i--) {
        ...
        let nextMeta = { path: `${meta.path}[${k}]`, level: meta.level + 1, parent: { _tag: sch._tag, type: sch.type } }
    ...
    case [T.TUPLE, T.UNION].includes(sch.type):
      for (let i = 0; i < sch.schs.length; i++) {
        let sch_ = sch.schs[i]
        let nextMeta = { path: `${meta.path}[][${i}]`, level: meta.level + 1, parent: { _tag: sch._tag, type: sch.type } }
        sch_ = walk(sch_, f, nextMeta)
    ...
    case [T.LIST].includes(sch.type):
      let nextMeta = { path: `${meta.path}[][0]`, level: meta.level + 1, parent: { _tag: sch._tag, type: sch.type } }
      ...
      switch (sch_.type) {
        case T.RECORD:
          for (let { k, sch, index } of ascRawSchs) {
            ...
            sch_.fields[k] = T.putAnchor(sch, sch_._tag)
        ...
        case T.TUPLE:
          for (let { k, sch, index } of ascRawSchs) {
            ...
            sch_.schs.splice(index, 0, T.putAnchor(sch, sch_._tag))
...
const changeType = (store, path, sch) =>
  update(store, path, (a, m) => {
    let newSch = T.putAnchor(sch, m.parent._tag)
    switch (true) {
