...
export {
  is, isContainer, walk, get, put, pop, move, update, bulkUpdate, getByAndUpdate, filterMostOuters,
...
const clone = (obj, f = a => a) => {
  ...
  walk(sch, (a, m) => f(putAnchor(() => a, { force: true }), m))
  return sch
}
...
const pop = (schema, path, indices) => {
  ...
  walk(schema, (sch_, meta) => {
    if (typeof path == "function" && !path(sch_, meta)) return sch_
    else if (typeof path == "string" && meta.path != path) return sch_
    else {
      sch_._halt = true
...
const put = (schema, path, rawSchs) => {
  ...
  const ChildrenTag = (sch, childSch) => {
    ...
    let tag = sheet?.children?.anyOf?.find(a => a.m == childSch.m && a.t == childSch.t)?.tag
    tag ||= sheet?.children?.tag
    return tag
  }
...
const changeT = (store, path, sch, opts = { force: false }) =>
...
const get = (currentNode, path) =>
  getByAndUpdate(currentNode, path, (a, m) => { readable(a, "_meta", m); return a })

const update = (currentNode, path, fupdate) =>
  getByAndUpdate(currentNode, path, fupdate)

const bulkUpdate = (currentNode, path, fupdate) => {
  let fget = (a, m) => m.path == path
  if (typeof path == "function") fget = path
...
const getByAndUpdate = (currentNode, path, fupdate) => {
  let fget = (a, m) => m.path == path
  if (typeof path == "function") fget = path
