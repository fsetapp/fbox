...
const clone = (obj, store) => {
  ...
  walk(sch, (a, m) => {
    let ogMeta
    if (store) ogMeta = store.schMetas[a.$anchor]
    const cloned = T.putAnchor(() => a, { force: true })

    if (ogMeta)
      Object.assign(store.schMetas, { [cloned.$anchor]: cloned.metadata })
...
const simplify = (sch) =>
  walk(JSON.parse(JSON.stringify(sch)), (a, m) => {
    let { type, $anchor, key, isEntry, index, parentAnchor } = a
    let common = { type, $anchor, key, index, parentAnchor }
    ...
    switch (a.type) {
      case T.RECORD:
        let { fields } = a
        return Object.assign(common, { fields })
...
const walk = (sch, f, meta = { path: "", level: 1, parent: {} }) => {
  ...
  const currentMeta = ({ type, $anchor, key }, { path, level }) =>
    ({ level, type, $anchor, key, path })

  switch (true) {
    case [T.RECORD].includes(sch.type):
      for (let i = sch.fields.length - 1; i >= 0; i--) {
      ...
      switch (sch_.type) {
        case T.RECORD:
          ...
          for (let { k, sch, index } of ascRawSchs) {
            ...
            k = k || `${taggedLV || "key"}_${Math.floor(Date.now() / 100)}`
            k = `${k}`
            for (let i = 0; i < sch_.fields.length; i++)
              if (sch_.fields[i].key == k) k = `${k} –`
              else continue

            index = boundIndex(index, sch_.fields)
            sch_.fields.splice(index, 0, T.putAnchor(sch))
            sch_.fields[index].key = k
            result.inserted.push({ k: sch_.fields[index].key, sch: sch_.fields[index], index: index })
          }
          break
        case T.LIST:
          break
        case T.TUPLE:
          for (let { k, sch, index } of ascRawSchs) {
            ...
            sch_.schs.splice(index, 0, T.putAnchor(sch))
...
const changeType = (store, path, sch) =>
  update(store, path, (a, m) => {
    ...
    const refChanged = newSch.type == T.REF && newSch.$ref != a.$ref
    const constChanged = newSch.type == T.VALUE && newSch.const != a.const
    const typeChanged = newSch.type != a.type

    const deniedChange = newSch.$ref == $anchor

    if (deniedChange) return a
    if (typeChanged || constChanged || refChanged)
      return Object.assign(newSch, { metadata: { title, description, rw, required }, $anchor, key })
    else
      return a
  })
