 ...
 */
const parentMeta = ({ m, t, $a, key, v, tag }, { path, lpath, level }) =>
  ({ m, t, $a, key, v, tag, level, path, lpath })

export const lPath = (lpath, append, m) => {
  let lpath_ = lpath.slice(0)
  if (m) readable(append, "_meta", m)
  lpath_.push(append)
  return lpath_
}
  ...
  switch (true) {
    case sch.hasOwnProperty("fields"):
      forEach(sch.fields, (sch_, i) => {
        let k = sch_.key
        let meta_ = { path: `${meta.path}[${k}]`, level: meta.level + 1, index: i, parent: parentMeta(sch, meta) }
        meta_.lpath = lPath(meta.lpath, sch_, meta_)

        sch_ = walk(sch_, f, meta_)
        sch.fields[i] = sch_
    ...
    case sch.hasOwnProperty("schs"):
      forEach(sch.schs, (sch_, i) => {
        let meta_ = { path: `${meta.path}[][${i}]`, level: meta.level + 1, index: i, parent: parentMeta(sch, meta) }
        meta_.lpath = lPath(meta.lpath, sch_, meta_)
    ...
    case sch.hasOwnProperty("sch"):
      let sch_ = sch.sch
      let meta_ = { path: `${meta.path}[][0]`, level: meta.level + 1, index: 0, parent: parentMeta(sch, meta) }
      meta_.lpath = lPath(meta.lpath, sch_, meta_)

      sch_ = walk(sch.sch, f, meta_)
      sch.sch = sch_
...
const put = (schema, id, rawSchs) => {
  ...
  const ChildrenTag = (sch, childSch) => {
    ...
    let tag = child?.of?.find(a => a.m == childSch.m && a.t == childSch.t)?.tag
    tag ||= child?.tag
    return tag
  }
        ...
        case sch_.hasOwnProperty("fields"):
          ...
          for (let { k, sch, index } of ascRawSchs) {
            ...
            if (!children(sch_)?.kDup) {
              k = k == 0 ? "0" : k
              k = k || `${newSch.tag || sch_.keyPrefix || "k"}${sch_.fields.length}`
              k = `${k}`
              while (keys[k]) k = `${k}-`
            }
            keys[k] = true
...
const putPoppedRawSchs = (store, { dstId, startIndex = 0 }, poppedPerSrc) => {
  ...
  if (inserted.length != 0) {
    ...
    readable(moved, meta.path, moved[sch.$a])
  }
...
const copyToRawSchs = (store, selectedPerParent, opts = {}) =>
  reduce(Object.values(selectedPerParent), (acc, cc) => {
    let idsMap = cc.reduce((acc, a) => acc.set(a.id, true), new Map)
    let collector = new Map

    getByAndUpdate(store, (a, m) => {
      if (idsMap.has(a.$a)) { readable(a, "_meta", m); collector.set(a.$a, a) }
      return collector.size == idsMap.size
    }, (a, m) => a)

    for (let c of cc) {
      if (opts.dstId && opts.dstId.startsWith(c.id)) continue
      let sch = collector.get(c.id)
...
const putSelectedRawSchs = (store, { dstId }, rawSchs) => {
  ...
  if (inserted.length != 0) {
    ...
    readable(pasted, meta.path, pasted[sch.$a])
  }
