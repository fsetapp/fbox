...
const move = (store, { isRefConstraint, dstPath, dstLevel, startIndex = 0 }, selectedPerParent) => {
  ...
  let poppedPerSrc = popToRawSchs(store, selectedPerParent, { dstPath, dstLevel, isRefConstraint })
...
const popToRawSchs = (store, selectedPerParent, opts = {}) => {
  ...
  for (let srcPath of srcPaths) {
    ...
    const beingRefed = (c) =>
      c.sch?.referrers && c.sch.referrers.length != 0 && opts.isRefConstraint && `${opts.dstLevel}` <= c.getAttribute("aria-level")
