...
const put = (schema, path, rawSchs) => {
  ...
  const ChildrenTag = (sch, childSch) =>
    schema.structSheet?.[sch.m].sheet(sch.t)?.children?.allowedSchs?.
      find(a => a.m == childSch.m && a.t == childSch.t)?.tag

  const selfTag = (sch) =>
    schema.structSheet?.[sch.m].sheet(sch.t)?.self?.tag
      ...
      switch (true) {
        case sch_.hasOwnProperty("fields"):
          ...
          for (let { k, sch, index } of ascRawSchs) {
            ...
            let tag = selfTag(newSch) || childTag(newSch)
