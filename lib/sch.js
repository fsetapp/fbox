      ...
      switch (sch_.type) {
        case T.RECORD:
          for (let { k, sch, index } of ascRawSchs) {
            ...
            sch_.fields[k] = T.putAnchor(sch, schema.put?.tag?.[sch_.tag])
        ...
        case T.TUPLE:
          for (let { k, sch, index } of ascRawSchs) {
            ...
            sch_.schs.splice(index, 0, T.putAnchor(sch, schema.put?.tag?.[sch_.tag]))
...
const changeType = (store, path, sch) =>
  update(store, path, (a, m) => {
    let newSch = T.putAnchor(sch, a._tag)
    switch (true) {
