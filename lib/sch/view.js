...
const viewFile = (assigns) =>
  html.for(assigns.sch, assigns.ui.rootKey + assigns.path)`
  <li id="${assigns.path}" .dataset="${{ tag: assigns.ui.taggedLevel[assigns.ui.level] }}" .key="${assigns.key}" .index="${assigns.ui.index}" aria-posinset="${assigns.ui.index + 1}" class="file" role="treeitem" aria-level="${assigns.ui.level}" aria-selected="false" tabindex="-1">
    ${viewKey(Object.assign(assigns, { key: wordBreakHtml(assigns.key) }))}
    ${viewType(assigns)}
    ${viewMeta(assigns)}
  </li>`
...
const viewKey = (assigns) =>
  assigns.ui.level == assigns.ui.rootLevel + 1 ? viewKeyRoot(assigns) : viewKeyNonRoot(assigns)
...
const viewType = (assigns) =>
  assigns.ui.level == assigns.ui.rootLevel + 1 ? viewTypeRoot(assigns) : viewTypeNonRoot(assigns)
