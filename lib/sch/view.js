...
export const renderRoot = (el, root, opts = {}) => {
  try {
    ...
    let ui = { level: 1, tab: 0, models: models, rootTag: root._tag, rootKey: root.key }

    viewMain(el, {
      sch: root,
      ui: Object.assign(ui, opts.ui),
      path: root.path || "", key: root.key, parent: { type: root.type, _tag: root._tag }
    })
  }
...
const viewModel = (assigns) => {
  switch (true) {
    case assigns.sch._tag == assigns.ui.rootTag: return viewRootItem(assigns)
...
const viewRootItem = (assigns) =>
  html.for(assigns.sch, assigns.ui.rootKey + assigns.path)`
  <li id="${assigns.path}" .dataset="${{ tag: assigns.sch._tag }}" .key="${assigns.key}" .index="${0}" aria-posinset="${1}" class="root-item" role="treeitem" aria-level="${assigns.ui.level}" aria-selected="false" tabindex="-1">
    <dfn class="h">
...
const viewFolder = (assigns) =>
  html.for(assigns.sch, assigns.ui.rootKey + assigns.path)`
  <li id="${assigns.path}" .dataset="${{ tag: assigns.sch._tag }}" .key="${assigns.key}" .index="${assigns.ui.index}" aria-posinset="${assigns.ui.index + 1}" class="folder" role="treeitem" aria-level="${assigns.ui.level}" aria-selected="false" tabindex="-1">
    <dfn class="h">
...
const viewFile = (assigns) =>
  html.for(assigns.sch, assigns.ui.rootKey + assigns.path)`
  <li id="${assigns.path}" .dataset="${{ tag: assigns.sch._tag }}" .key="${assigns.key}" .index="${assigns.ui.index}" aria-posinset="${assigns.ui.index + 1}" class="file" role="treeitem" aria-level="${assigns.ui.level}" aria-selected="false" tabindex="-1">
    ${viewKey(Object.assign(assigns, { key: wordBreakHtml(assigns.key) }))}
    ${viewType(assigns)}
    ${viewMeta(assigns)}
  </li>`
...
const viewKeyed = (assigns) =>
  assigns.sch.order.map((k, i) =>
    viewModel({
      ...
      parent: { type: assigns.sch.type, path: assigns.path, _tag: assigns.sch._tag },
...
const viewKey = (assigns) =>
  assigns.parent._tag == assigns.ui.rootTag ? viewKeyRoot(assigns) : viewKeyNonRoot(assigns)
...
const viewType = (assigns) =>
  assigns.parent._tag == assigns.ui.rootTag ? viewTypeRoot(assigns) : viewTypeNonRoot(assigns)
