...
export const renderMeta = (container, sch, root) => {
  if (sch._tag != T.FMODEL_TAG && sch._tag != undefined) return

  try {
    render(container, html.for(sch)`
    <p>${slashedPath(sch.rootKey, sch.path) || "-"} : <span>${sch.type}</span></p>
...
const textInput = (sch, key, opts = {}) =>
  labelA(sch, key, html`
    <textarea name="${name(sch, key)}" maxlength="${opts.maxlength}" minlength="${opts.minlength}" ?readonly="${opts.readonly}" rows="1" spellcheck="false" class="${errors(sch, key) && "invalid"}" oninput="${autoResize}">${value(sch, key) || opts.value}</textarea>
  `, opts)

const numberInput = (sch, key, opts = {}) =>
  labelA(sch, key, html`
    <input type="number" name="${name(sch, key)}" inputmode="decimal" step="any" min="${opts.min}" max="${opts.max}" value="${value(sch, key) || opts.value}" ?readonly="${opts.readonly}" class="${errors(sch, key) && "invalid"}">
  `, opts)

const integerInput = (sch, key, opts = {}) =>
  labelA(sch, key, html`
    <input type="number" name="${name(sch, key)}" inputmode="numeric" pattern="[0-9]*" min="${opts.min}" max="${opts.max}" value="${value(sch, key) || opts.value}" ?readonly="${opts.readonly}" class="${errors(sch, key) && "invalid"}">
  `, opts)

const boolInput = (sch, key, opts = {}) =>
  labelA(sch, key, html`
    <input type="checkbox" name="${name(sch, key)}" ?disabled="${opts.readonly}" ?checked="${value(sch, key) || opts.value}">
  `, opts)

const enumInput = (sch, key, opts = {}) =>
  labelA(sch, key, html`
    <select name="${name(sch, key)}" ?disabled="${opts.readonly}">
      ${(opts.collection).map(([v, t]) => html`<option value="${v}" ?selected="${(value(sch, key) || opts.selected) == v}">${t}</option>`)}
    </select>
  `, opts)
...
const renderTypeMeta = (sch) => {
  ...
  if (sch._meta.parent.type == T.RECORD && ![T.FMODEL_TAG, "file", "project"].includes(sch._tag))
