...
const reorderDiffedSchs = (store, schs) => {
  ...
  for (let reorderedSch of schs) {
    let { pa, ...reorderedSch_ } = reorderedSch
    let parent = parentMap.get(reorderedSch.pa)
    if (!parent) { parent = []; parentMap.set(reorderedSch.pa, parent) }

    parent.push(reorderedSch_)
  }

  const reorder = (a, arr) => {
    let arrMap = {}
    for (let sch of arr) arrMap[sch.$a] = sch
    // In case coming schs does not exist in `arr`, return a (reorderedSch_)
    return parentMap.get(a.$a).sort((a, b) => a.index - b.index).map(a => arrMap[a.$a] || a)
  }
