...
export const diff = (currentSch, baseSch, opts = {}) => {
  ...
  Sch.walk(currentSch, (newSch, m) => {
    ...
    if (foundSch) {
      ...
      if (Object.keys(_diff).length != 0) {
        readable(newSch, "_diff", Object.assign(_diff, { meta: { level: m.level, path: m.path, pa: m.parent.$a } }))
        diffs.push(newSch)
      }
      ...
      readable(newSch, "_diff", { [NEW]: { parent: m.parent }, meta: { level: m.level, path: m.path, pa: m.parent.$a } })
      diffs.push(newSch)
    }
  ...
  Sch.walk(baseSch, (a, m) => {
    if (!a._visit) {
      readable(a, "_diff", { [REMOVED]: { parent: m.parent }, meta: { level: m.level, path: m.path, pa: m.parent.$a } })
      removed.push(a)
...
const updateFromDiff = (store, { fmodels, files }) => {
  updateDiffedSchs(store, files, file => fileToStore(file))
  updateDiffedSchs(store, fmodels)
}

const updateDiffedSchs = (store, schs) => {
  let schUpdateMap = {}
  for (let updatedSch of schs)
    schUpdateMap[updatedSch.$a] = updatedSch

  Sch.walk(store, (a, m) => {
    if (Object.keys(schUpdateMap).length == 0) a._halt = true
    if (schUpdateMap[a.$a]) {
      a = Sch.copy(schUpdateMap[a.$a])
      delete schUpdateMap[a.$a]
    }
    return a
  })
  ...
  let schMap = {}
  for (let updatedSch of schs)
    Sch.walk(updatedSch, (a, m) => { schMap[a.$a] = a; return a })

  // Merge baseStore indices
  if (store._indices)
    Sch.walk(store, (a, m) => {
...
const remove_diffed_schs = (store, schs) => {
  ...
  for (let removedSch of schs) {
    let { key, index, ...sch } = removedSch
    parentMap[removedSch.pa] ||= []
    parentMap[removedSch.pa].push(sch.$a)
  }
...
const insertDiffedSchs = (store, schs, schf = a => a) => {
  ...
  for (let addedSch of schs) {
    let { key, index, ...sch } = addedSch
    parentMap[addedSch.pa] ||= []
    parentMap[addedSch.pa].push({ k: key, sch: () => Sch.copy(schf(sch)), index: index })
  }
