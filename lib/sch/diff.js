...
const updateFromDiff = (store, { fmodels, files }) => {
  updateDiffedSchs(store, files, (file0, file1) => {
    let { fields, ...file } = Sch.copy(fileToStore(file1))
    return Object.assign(file0, file)
  })
  updateDiffedSchs(store, fmodels, (fmodel0, fmodel1) => fmodel1)
}

const updateDiffedSchs = (store, schs, shhf = a => a) => {
  let schUpdateMap = {}
