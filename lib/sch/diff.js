...
// TODO: optimization, it seems possible to update via indices directly without walking the whole project (please write test first)
// status: patched (needs a real implementation)
const updateDiffedSchs = (store, schs, topSchf = (old, new_) => new_) => {
  ...
  for (let updatedSch of schs)
    Sch.walk(updatedSch, (a, m) => {
      let found = store._indices[a.$a]
      if (found) {
        let metadata = found.metadata
        if (metadata) writable(a, "metadata", metadata)
