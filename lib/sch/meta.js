...
const validate = (sch, metaForm) => {
  switch (sch.t) {
    ...
    case T.UINT16:
    case T.UINT32:
      validateTypeNumber(sch, metaForm, { min: sch.min, max: sch.max })
      break
    case T.FLOAT32:
    case T.FLOAT64:
...
const validateTypeNumber = (sch, metaForm, opts) => {
  ...
  validateMinMax(sch, metaForm)
  validateNumber(sch, "default", metaForm, takeAssert(sch, ["min", "max", "multipleOf"]))
}
...
export const examples = (sch) => {
  ...
  if (sch.t == T.RECORD && sch.fields.length == 0)
    sch.examples

  else if (sch.t == T.RECORD) {
    let acc = {}
  ...
  else if ([...T.NUMBER, ...T.INTEGER].includes(sch.t)) {
    let min = sch.min || 0
  ...
  else if (sch.t == T.BOOLEAN)
    sch.examples = [[true, false][randInt(2)]]

  else if (sch.t == T.NULL)
    sch.examples = [null]
