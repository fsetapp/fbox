...
const addSch = ({ tree, store, e }) => {
  ...
  if (typeof e.target.putSch == "function") {
    ...
    putSch = anyOf.find(a => is(a, e.target.putSch()))
  }
  else
    putSch =
...
const activateEditText = ({ tree, store, e }) => {
  ...
  let textRect = tree._walker.currentNode.querySelector(".v")
  if (textRect)
    activateEdit({ tree, store, editMode: "editVal", textRect })
}
...
const activateEditType = ({ tree, store, e }) => {
  ...
  if (textRect)
    activateEdit({ tree, store, e, editMode: "editType", textRect })
}
...
const changeTSelected = (tree, store, textArea) => {
  ...
  if (allowedSch) Sch.changeT(store, textArea._treeItem.id, () => Sch.clone(allowedSch))
  else if (anchor) Sch.changeT(store, textArea._treeItem.id, () => Core.ref(anchor))
...
const changeVSelected = (tree, store, textArea) => {
  ...
  if (allowedSch) {
    if (allowedSch.m == itemSch.m && allowedSch.t == itemSch.t)
      Sch.changeT(store, textArea._treeItem.id, () => Sch.clone(allowedSch))
    else {
      // Make sure only `v` is changed
      Object.assign(allowedSch, { m: itemSch.m, t: itemSch.t })
...
const isContainer = sch => sch?.fields || sch?.schs
...
const wrapBefore = ({ tree, store }) =>
  multiParentsOp({ tree, store, sort: "asc" }, (pPath, grouped, { sorted }) => {
    if (!sorted[0].previousElementSibling) return {}
    if (!isContainer(sorted[0].previousElementSibling.sch)) return {}
    return Sch.move(store, {
      dstPath: sorted[0].previousElementSibling.id,
...
const multiParentsOp = ({ tree, store, sort }, f) => {
  ...
  for (let srcPath of Object.keys(indicesPerParent)) {
    let sorted
    if (sort == "asc") sorted = indicesPerParent[srcPath].sort((a, b) => a.index - b.index)
    else if (sort == "desc") sorted = indicesPerParent[srcPath].sort((a, b) => b.index - a.index)

    result = f(srcPath, indicesPerParent, { sorted })
  }
...
const suggestSchsFitParent = (tree, store) => {
  ...
  switch (true) {
    case !!parentRule?.nthChild:
      return parentRule.nthChild[currentSch._meta.index].anyOf
    case !!parentRule?.children:
      return parentRule.children.anyOf
    default:
      return null
  }
}
