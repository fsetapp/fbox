...
const activateEditType = ({ tree, store }) => {
  ...
  activateEdit({ tree, store, editMode: "editType", textRect })
}
const activateEdit = ({ tree, store, editMode, textRect }) => {
  let currentNode = tree._walker.currentNode

  const sch = Sch.update(store, currentNode.id, (a, m) => writable(a, "uiMode", editMode))
  tree._render(store)
...
const changeTypeSelected = (tree, store, textArea) => {
  ...
  let allowedSch = allowedSchs_.find(sch_ => sch_.t == textArea.value)
  let anchor = store._models[textArea.value] && textArea.value || Object.keys(store._models).find(anchor => store._models[anchor].display == textArea.value)
...
const paste = ({ tree, store, srcStore }) => {
  ...
  if (![T.RECORD, T.UNION, T.TUPLE].includes(dstSch.t)) return
  ...
  if (window._treeClipboard) {
    ...
    const { type, srcStoreKey, ops, selected } = window._treeClipboard
    let selectedPerParent = selected
    let result = {}

    if (store.key == srcStoreKey)
      switch (type) {
        case "copy":
