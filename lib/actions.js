...
const changeTypeSelected = (tree, store, textArea) => {
  ...
  let anchor = store._models[textArea.value] && textArea.value || Object.keys(store._models).find(anchor => {
    ...
    return allowedSchs_.find(sch_ => {
      const isRef = sch_.t == T.ref(null).t

      if (isRef)
        if (!sch_.to) return true
        else return sch_.to.t == model.sch.t
    })
  })
...
const cut = ({ store, tree }) => {
  ...
  window._treeClipboard = {
    ...
    ops: ({ dstPath, dstLevel }) => Sch.popToRawSchs(store, selectedPerParent, { dstPath, dstLevel }),
    ...
    if (store.key == srcStoreKey)
      switch (type) {
        case "copy":
          ...
          break
        case "cut":
          result = Sch.move(store, { dstPath, dstLevel: dstSch._meta.level }, selectedPerParent)
          break
      }
    else
      switch (type) {
        case "copy":
          ...
          break
        case "cut":
          let poppedPerSrc = ops({ dstPath: "", dstLevel: dstSch._meta.level })
