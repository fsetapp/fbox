...
const addSch = ({ tree, store, e }) => {
  ...
  let { allowedSchs, put, defaultSch } = suggestChildren(tree, store)
  if (!allowedSchs) return

  let putSch =
    allowedSchs[e.target.putSch] ||
    allowedSchs[defaultSch] ||
    allowedSchs[0]
  if (!putSch) return
...
const addSection = (arg) => addSch(arg)
const addForm = (arg) => addSch(arg)
...
const changeVSelected = (tree, store, textArea) => {
  ...
  let allowedSch = allowedSchs_.reduce((acc, sch_) => {
    let valSch = castVal(sch_)(sch_, textArea.value)
    if (valSch) return valSch
    else return acc
  }, false)
