...
const addSch = ({ tree, store, e }) => {
  ...
  let childrenRules_ = childrenRules(tree, store)
  if (!childrenRules_) return

  let defaultSchIndex = e.target.defaultSch || childrenRules_.defaultSch || 0
  let defaultSch = childrenRules_.allowedSchs[defaultSchIndex]
...
const activateEditType = ({ tree, store, e }) => {
  ...
  let allowedTs = allowedSchsFitParent(tree, store)
...
const changeTypeSelected = (tree, store, textArea) => {
  const allowedSchs_ = allowedSchsFitParent(tree, store)
  const { toStr } = store.structSheet[store.ext]

  let allowedSch = allowedSchs_.find(sch_ => toStr(sch_.t) == textArea.value)
...
const paste = ({ tree, store, srcStore }) => {
  ...
  const allowedSchs_ = allowedSchs(tree, store)
  if (!allowedSchs_)
    return
...
const childrenRules = (tree, store) => {
  let { parentSch, currentSch } = currentParent(tree, store)
  const { sheet } = store.structSheet[store.ext]

  for (let row of sheet || []) {
    switch (true) {
      case match(currentSch, row.selector):
        if (row.rules.nthChild) {
          const children = Object.values(row.rules.nthChild)
          const schs = reduce(children, (acc, a) => {
            reduce(a.allowedSchs, (acc0, a0) => {
              acc0.push(a0)
            }, acc)
          }, [])
          return { allowedSchs: schs }
        }
        else
...
const allowedSchs = (tree, store) =>
  childrenRules(tree, store)?.allowedSchs

const allowedSchsFitParent = (tree, store) => {
  let { parentSch, currentSch } = currentParent(tree, store)
  const { sheet } = store.structSheet[store.ext]
  ...
  for (let row of sheet || [])
    switch (true) {
      ...
      case match(parentSch, row.selector):
