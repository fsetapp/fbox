...
const addSch = ({ tree, store, e }) => {
  ...
  let { of: anyOf, put, defaultSch } = suggestChildren(tree, store)
  if (!anyOf) return
...
const paste = ({ tree, store, srcStore }) => {
  ...
  const allowedSchs_ = suggestChildren(tree, store).of
  if (!allowedSchs_)
    return
...
const suggestChildren = (tree, store) => {
  ...
  // let rule = suggest(currentRule, nextChildIndex)
  // if (rule.of) rule.of = rule.of.map(a => a == "all" ? structSheet.C : structSheet.C[a])
  // if (rule.defaultSch) rule.defaultSch = structSheet.C[rule.defaultSch]
...
const suggestSchsFitParent = (tree, store) => {
  ...
  return suggest(parentRule, currentSch.index)?.of
}
const suggest = (rule, childIndex) => {
  switch (true) {
    case !!rule?.child:
      return rule.child[childIndex] || rule.children || {}
    case !!rule?.children:
...
const preCompileCheck = (tree, store) => {
  ...
  const annotate = (parentRule, current) => {
    const nthChild = parentRule?.child?.[current._local.index]?.of
    const children = parentRule?.children?.of

    if (nthChild)
      if (!nthChild.find(a => allow(a, current)))
        Object.assign(current, { terror: nthChild })
    if (children)
