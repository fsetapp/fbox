...
export const fromProject = (project, { imports }) => {
  ...
  const structSheet = reduce(imports, (acc, pkg) => assign(acc, pkg.deps), {})
  const controllers = reduce(imports, (acc, pkg) => assign(acc, pkg.controller), {})
  const elements = reduce(imports, (acc, pkg) => assign(acc, pkg.elements), {})
  const diffableActs = reduce(imports, (acc, pkg) => assign(acc, pkg.diffableActs), {})
