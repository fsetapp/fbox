...
export const fromProject = (project, { imports }) => {
  // Merge all top sheets from imports to proj's sheet
  reduce(imports, (acc, mod) => assign(acc, mod.topSheet), Proj.structSheet.sheet)
  imports.push({
    structSheet: { [s(Proj).t]: Proj.structSheet },
    controller: { [Proj.PROJECT_TAG]: controller }
  })

  const structSheet = reduce(imports, (acc, mod) => assign(acc, mod.structSheet), {})
  const controllers = reduce(imports, (acc, mod) => assign(acc, mod.controller), {})
  const elements = reduce(imports, (acc, mod) => assign(acc, mod.elements), {})

  let store = Core.putAnchor(() => Proj.project({
    structSheet,
    controllers,
    elements,
    refParentLevel: 2,
...
export const buildFolderTree = (projectStore) => {
  ...
  let lpathList = files.map(file => {
    ...
    assign(file_, { tag: Proj.FILE_TAG })
    // Backward compat just in case
    forEach(file_.fields, (a, i) => assign(a, { tag: Core.TOPLV_TAG }))
