...
// Important note: a "diff" that is MOVED or REMOVED will contain stale object.
// Finding a toplv sch from `oldParent` or `_diff.meta.*` will have stale information
// from base store, not the current one. This aspect forgotten will cause bugs easily.
...
export const taggedDiff = (projectStore, f) => {
  ...
  let findFmodelMap = {}
  const getFreshTop = (topId) => {
    let cached = projectStore._indices?.[topId] || findFmodelMap[topId]
  ...
  const simpleFile = ({ $a, key, tag, index, m, t, _diff }, lpath) => {
    lpath ||= _diff.meta.lpath
    // This is a reverse op of #buildFolderTree: lpath.unshift(projectStore) (server state -> client state: wrap projectStore)
    // So reverse op here is (client state -> server state: unwrap projectStore). Also pop itself out. Basically trimming lead and tail.
    lpath.shift()
    lpath.pop()
    let lpath_ = lpath.map(({ key, index, m, t, $a, tag }) => ({ key, index, m, t, $a, tag }))
    return ({ $a, key, tag, index, m, t, lpath: lpath_, pa: projectStore.$a })
  }
  ...
  forEach(toUpdateSchs.folders, folder => {
    if (folder._diff[Diff.NEW]) {
      ...
      lpath.push(keep)
      added.files[keep.$a] = simpleFile(keep, lpath)
    }
    if (folder._diff[Diff.NEW_KEY]) {
      folder._diff.meta.lpath.shift()
      let baseLpath = folder._diff.meta.lpath

      // New folder keys will also trigger update all descendant files' lpath.
      walkFile(folder, (file, m) => {
        let lpath = m.lpath
        lpath.splice(0, 0, ...baseLpath)

        changed.files[file.$a] = simpleFile(file, lpath)
        return file
      })
    }
    ...
    // Note: file order update is in `changed.files` scope, associated fact in diff scope is quite fragile
      ...
      switch (oldParent.tag) {
        ...
        default:
          ...
          fmodelOf = getFreshTop(staleFmodel.$a)
  ...
  for (let i = 0; i < toUpdateSchs.subfmodels.length; i++) {
    ...
    if (subfmodel._diff[Diff.NEW] ||
      ...
      // fresh* except for Diff.REMOVED, it's stale.
      let freshFile = subfmodel._diff.meta.lpath.find(a => a.tag == FILE_TAG)
      let freshFmodel = subfmodel._diff.meta.lpath.find(a => a.tag == TOPLV_TAG)

      if (subfmodel._diff[Diff.REMOVED]) freshFmodel = getFreshTop(freshFmodel.$a)
      fmodelPath = freshFmodel._meta.path
      if (!added.fmodels[fmodelPath] && !removed.fmodels[fmodelPath]) {
        changed.fmodels[fmodelPath] ||= assign(freshFmodel, { pa: freshFile.$a })
      }
    }
