...
import { PROJECT, FOLDER, FILE_TAG, PROJECT_TAG, KEEP_EXT } from "../pkgs/proj.js"
...
export const taggedDiff = (projectStore, f) => {
  ...
  const simpleFile = ({ $a, key, tag, index, m, t, _diff }) => {
    let lpath = _diff.meta.lpath.map(({ key, m, t, $a, tag }) => ({ key, m, t, $a, tag }))
  ...
  for (let sch of diffs) {
    ...
    else if (sch.t == KEEP_EXT) continue
    else if (sch.t == FOLDER) toUpdateSchs.folders.push(sch)
    else if (sch.tag == FILE_TAG) toUpdateSchs.files.push(sch)
    else if (sch.tag == TOPLV_TAG) toUpdateSchs.fmodels.push(sch)
    else if (![PROJECT, FOLDER, KEEP_EXT].includes(sch.t) &&
  ...
  forEach(toUpdateSchs.folders, folder => {
    if (folder._diff[Diff.NEW]) {
      ...
      let innerMostFolder = lpath.pop()
      let keep = innerMostFolder.fields.find(a => a.t == KEEP_EXT)

      lpath.push(innerMostFolder)
      keep.lpath = lpath.map(a => ({ ...a, fields: [] }))
      added.files[keep.$a] = keep
    }
    if (folder._diff[Diff.NEW_KEY]) {
      folder._diff.meta.lpath.shift()
      let baseLpath = folder._diff.meta.lpath.map(a => ({ ...a, fields: [] }))

      walkFile(folder, (file, m) => {
        m.lpath.shift()
        let lpath = m.lpath.map(a => ({ ...a, fields: [] }))
        let innerMost = lpath.pop()
        if (innerMost.t != KEEP_EXT) lpath.push(innerMost)
