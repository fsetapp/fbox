...
const handleTreeKeydown = (store) => function (e) { handleTreeKeydown_(e, this, store) }
function handleTreeKeydown_(e, tree, store) {
...
const handleTextAreaKeyDown = (e, tree, store) => {
  ...
  command && command({ tree, store }, textArea)
...
const addSch = ({ tree, store }) => {
  ...
  if (currentNode.classList.contains("fmodels"))
    Sch.put(store, currentNode.id, [{ k: null, sch: T.record, index: 0 }])
  else
    Sch.put(store, currentNode.id, [{ k: null, sch: T.string, index: Number.MAX_SAFE_INTEGER }])
...
const changeTypeSelected = (tree, store, textArea) => {
  let newSch = store._allSchs.filter(sch_ => sch_().type == textArea.value)[0]
...
const init = ({ store, treeSelector, metaSelector }) => {
  View.renderRoot(store)
  ...
  if (tree) {
    ...
    tree.onkeydown = handleTreeKeydown(store)
