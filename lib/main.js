...
const createStore = (opts = {}) => {
  ...
  if (opts.addDefault) s._addSchDefault = s._allowedSchs.find(sch_ => sch_.type == opts.addDefault().type)
...
const isModelChangedCmd = (cmdname) => [
  addSch, deleteSelected, submitEdit
].map(cmd => cmd.name).includes(cmdname)
...
const addSch = ({ tree, store }) => {
  ...
  let defaultSch = parent._addSchDefault || store._allowedSchs[0]
  let addingSch = () => Sch.clone(defaultSch)

  if (parent.put?.pos == "prepend") index = 0
  else index = Number.MAX_SAFE_INTEGER
  Sch.put(store, currentNode.id, [{ k: null, sch: addingSch, index: index }])
