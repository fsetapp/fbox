...
const createStore = (opts = {}) => {
  ...
  s._allowedSchs = opts.allowedSchs || allSchs.map(sch => sch())

  if (opts.tag) s._tag = opts.tag
  if (opts.addDefault) s._addSchDefault = s._allowedSchs.find(sch_ => sch_.type == opts.addDefault().type) && opts.addDefault()
...
async function handleTreeKeydown_(e, tree, store, opts) {
  ...
  tree.dispatchEvent(new CustomEvent("tree-command", {
    bubbles: true,
    detail: {
      file: fileItem?.key, path: tree._walker.currentNode.id, command: command
    }
  }))
}
...
const renderSchMeta = ({ tree, store }) => {
  ...
  if (selected.length == 1) {
    ...
    sch.displayKey = `${selected[0].key}`
    sch.path = selected[0].id
    sch.rootKey = tree.querySelector(".root-item").key
    sch.metadata = store.metaTable[sch.$anchor]
...
const addSch = ({ tree, store }) => {
  ...
  let defaultSch =
    ...
    (parent._allowedSchs && parent._allowedSchs[0]) ||
    (store._allowedSchs && store._allowedSchs[0])
...
const changeTypeSelected = (tree, store, textArea) => {
  let allowedSch = store._allowedSchs.find(sch_ => sch_.type == textArea.value)
...
const update = ({ store, detail, target }) => {
  ...
  Object.assign(store.metaTable, { [sch.$anchor]: sch.metadata })
