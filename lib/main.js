...
const createStore = (opts = {}) => ({ ...T.putAnchor(T.record), _box: opts.tag || T.FMODEL_BOX, _allSchs: opts.allowedSchs || allSchs })
...
const addSch = ({ tree, store }) => {
  ...
  if (parent.put == "append") index = 0
  else index = Number.MAX_SAFE_INTEGER
  Sch.put(store, currentNode.id, [{ k: null, sch: parent._allSchs[0], index: index }])
...
const initFileView = ({ store, target, metaSelector }) => {
  component({
    ...
    opts: {
      ...
      treeKeyDownOpts: { denyCmds: ["activateEditType"] },
      focus: true
    }
  })
}
