...
export const MODULE = s_({ PKG_NAME }).t
...
const ctorVoid = (ctor_, opts) => Object.assign(ctor_, { m: MODULE }, opts)

const html = (opts) => ctor({ t: HTML }, opts)
const base = (opts) => ctorVoid({ t: BASE }, opts)
const head = (opts) => ctor({ t: HEAD }, opts)
const link = (opts) => ctorVoid({ t: LINK }, opts)
const meta = (opts) => ctorVoid({ t: META }, opts)
...
const hr = (opts) => ctorVoid({ t: HR }, opts)
...
const br = (opts) => ctorVoid({ t: BR }, opts)
...
const wbr = (opts) => ctorVoid({ t: WBR }, opts)
const area = (opts) => ctorVoid({ t: AREA }, opts)
const audio = (opts) => ctor({ t: AUDIO }, opts)
const img = (opts) => ctorVoid({ t: IMG }, opts)
const map = (opts) => ctor({ t: MAP }, opts)
const track = (opts) => ctorVoid({ t: TRACK }, opts)
const video = (opts) => ctor({ t: VIDEO }, opts)
const embed = (opts) => ctorVoid({ t: EMBED }, opts)
...
const source = (opts) => ctorVoid({ t: SOURCE }, opts)
...
const col = (opts) => ctorVoid({ t: COL }, opts)
...
const input = (opts) => ctorVoid({ t: INPUT }, opts)
...
// Internal
const transparent = (opts) => ctor({ t: "transparent" }, opts)
...
].reduce((acc, sch) => { let a = sch(); acc[a.t] = a; return acc }, {})
// {
//   audio: {
//     c: []
//     m: ["flow"],
...
export const tops = { anyOf: all.map(a => a({ tag: Core.TOPLV_TAG })) }
...
const sheet = {
  ...
  [HEAD]: {
    self: {
      off: [Actions.reorderDown],
    },
    children: {
      anyOf: [title({ n: 1 }), base(), link(), meta(), style(), script()]
    }
  },
  [TITLE]: {
    children: { anyOf: [] }
  ...
  [AUDIO]: {
    ...
    // transparent is like delegating rule to parent (and so on if parent is also transparent)
    // ge: greater than or equal to
    // seq: validate in sequance
    // desnot: descendant not any of these
    // these flags should be removed once rules compile
    children: {
      case: [
        { has: ["src"], do: { seq: [track({ ge: 0 }), transparent({ desnot: [video(), audio()] })] } },
        { do: { anyOf: [] } }
      ]
    }
  },
