...
export const taggedUnion = (opts) => ctor({ t: TAGGED_UNION, fields: [{ ...putAnchor(record), key: "tag_a" }], tagname: "tag", anyOf: [record()], tag: "tag" }, opts)
export const dict = (opts) => ctor({ t: DICT, schs: [putAnchor(string), putAnchor(any)] }, opts)
export const value = (strval) => {
  let scalarT = { number: FLOAT64, string: STRING, boolean: BOOLEAN }
  try {
    ...
    if (val == null)
      ...
      return { m: MODULE, t: scalarT[typeof val], v: val }
    else
      return
  }
  catch (e) { return }
...
const toVal = ({ t, v }, strval) => {
  ...
  const tToType = { [FLOAT64]: "number", [STRING]: "string", [BOOLEAN]: "boolean" }
