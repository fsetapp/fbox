...
const define = (name, store, tree, domEvents) => {
  const on = instance => {
    instance.listeners = new Map(domEvents)

    for (let [name, listener] of instance.listeners) {
      listener = e => handleDomEvent(e, tree, store, domEvents)
...
export const TreeComponent = (params) => {
  ...
  const { treeKeyDownCmds, textAreaKeyDownCmd, dblClickCmd, clickCmds, domEvents, opts = {} } = params
  ...
  if (tree) {
    ...
    define(target.tagName.toLowerCase(), store, tree, domEvents)
...
const handleDomEvent = (e, tree, store, cmds = []) => {
  const cmdsMap = new Map(cmds)
