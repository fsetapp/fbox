...
const shouldReturn = (tree, store, command) => {
  ...
  const isBlocked = (schRule, commandName) => {
  ...
  const currentRule = store.structSheet[currentSch.m]?.sheet(currentSch.t)
  const parentRule = store.structSheet[parentSch.m]?.sheet(parentSch.t)

  return isBlocked(currentRule, cmd) || isBlocked(parentRule, cmd) || false
}
